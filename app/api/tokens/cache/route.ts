...
async function tryTokenVariations(symbol: string): Promise<string | null> {
  const variations = [
    ...
    symbol.toLowerCase() + '-gold',
    symbol.toLowerCase() + '-network',
    symbol.toLowerCase() + '-index',
  ]

  for (const variation of variations) {
    const imageUrl = await fetchFromCoinGecko(variation)
    if (imageUrl) {
      return imageUrl
    }
  }
...
export async function GET(request: NextRequest) {
  ...
  // Fetch images for all symbols (parallelized)
  await Promise.all(
